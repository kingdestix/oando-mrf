...
const app = {
    ...
    requireRole(role) {
      ...
      if (!user || user.role !== role) {
        ...
        setTimeout(() => {
          ...
          } else if (['pod_planner', 'technical_coordinator', 'assistant_manager', 'area_manager_land', 'area_manager_swamp', 'area_manager_phc', 'discipline_unit', 'discipline_manager', 'dodm'].includes(userRole)) {
    ...
    /**
     * Check if user can view commercial details (prices, amounts)
     * Field personnel (worker, technical_coordinator, assistant_manager, area_manager_*) cannot see commercial details
     */
    canViewCommercialDetails() {
      const user = this.getUser();
      if (!user) return false;
      
      const fieldPersonnel = ['worker', 'technical_coordinator', 'assistant_manager', 'area_manager_land', 'area_manager_swamp', 'area_manager_phc'];
      return !fieldPersonnel.includes(user.role);
    },
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (navbarMenu && user.role === 'admin') {
      ...
      `;
    } else if (navbarMenu && user.role === 'pod_planner') {
      navbarMenu.innerHTML = `
        <a href="/approval-dashboard.html" class="nav-link">Approval Dashboard</a>
      ...
      navbarMenu.innerHTML = `
        ...
        <a href="/profile.html" class="nav-link">My Profile</a>
      `;
    } else if (navbarMenu && ['technical_coordinator', 'assistant_manager', 'area_manager_land', 'area_manager_swamp', 'area_manager_phc', 'discipline_unit', 'discipline_manager', 'dodm'].includes(user.role)) {
      navbarMenu.innerHTML = `
        <a href="/approval-dashboard.html" class="nav-link">Approval Dashboard</a>
        <a href="/profile.html" class="nav-link">My Profile</a>
      `;
    ...
    // Mobile sidebar toggle - handled by global functions
    // toggleSidebar() and closeSidebar() defined below
  });
  ...
  function toggleSidebar() {
    const sidebar = document.getElementById('sidebarMenu') || document.querySelector('.navbar-menu');
