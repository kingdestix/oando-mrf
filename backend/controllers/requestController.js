...
const fs = require('fs');
const { generateMRFPDF } = require('../utils/invoiceGenerator');
...
async function createRequest(req, res) {
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      // Note: material_class will be stored in material_category if column doesn't exist
      const requestResult = await client.query(
        `INSERT INTO material_requests (
          ...
          workflow_stage, area, workflow_location, remarks
        ...
        [
          ...
          data.unit_tag || '',
          data.discipline,
          // Store material_class and material_category - if material_class exists, combine them
          data.material_class && data.material_category 
            ? `${data.material_class} - ${data.material_category}`
            : data.material_class || data.material_category || '',
          ...
          req.user.id,
          'TECHNICAL_COORDINATOR_REVIEW', // Initial workflow stage - goes to tech coordinator first
          data.area || null, // Store area
          assetValue, // Store location for workflow routing
          data.remarks || null
        ]
      );
...
async function getRequests(req, res) {
  try {
    ...
    if (month) {
      ...
      whereClause.push(`EXTRACT(YEAR FROM r.request_date) = $${paramIndex}`);
      params.push(year);
      paramIndex++;
      whereClause.push(`EXTRACT(MONTH FROM r.request_date) = $${paramIndex}`);
    ...
    // Also check for area managers - they should see all requests for their area
    const user = req.user;
    if (user && user.role && user.role.startsWith('area_manager_')) {
      const areaType = user.role.replace('area_manager_', '').toUpperCase();
      if (areaType === 'LAND') {
        whereClause.push(`(r.area = $${paramIndex} OR r.mrf_number LIKE $${paramIndex + 1})`);
        params.push('Land Area', 'LAR-%');
        paramIndex += 2;
      } else if (areaType === 'SWAMP') {
        whereClause.push(`(r.area = $${paramIndex} OR r.mrf_number LIKE $${paramIndex + 1})`);
        params.push('Swamp Area', 'SAR-%');
        paramIndex += 2;
      } else if (areaType === 'PHC') {
        whereClause.push(`(r.area = $${paramIndex} OR r.mrf_number LIKE $${paramIndex + 1})`);
        params.push('PHC POD', 'PHC-%');
        paramIndex += 2;
      }
    ...
    // POD planners and admins can see all requests
    if (req.user.role === 'worker') {
      whereClause.push(`r.user_id = $${paramIndex}`);
      params.push(req.user.id);
      paramIndex++;
    } else if (req.user.role === 'pod_planner' || req.user.role === 'admin') {
      // POD and admin can see all requests - no additional filter needed
    } else if (user_id) {
...
async function getLookups(req, res) {
  try {
    // Fixed disciplines list (6 disciplines as per worker dashboard)
    const fixedDisciplines = [
      { group_name: 'ROTATING EQUIPMENT' },
      { group_name: 'POWER GENERATION' },
      { group_name: 'METERING AND AUTOMATION' },
    ...
    res.json({
      ...
      lookups: {
        ...
        swampAreaLocations: ['OGBOINBIRI', 'BRASS', 'OBAMA', 'CLOUGH CREEK', 'BRASS TERMINAL', 'SAMABRI'],
        phcPodLocations: ['OBOB', 'KWALE', 'IRRI', 'OSHIE', 'EBOCHA', 'IDU', 'AKRI', 'OGBOINBIRI', 'BRASS', 'OBAMA', 'CLOUGH CREEK', 'BRASS TERMINAL', 'SAMABRI', 'TEBIDABA', 'PHC']
      }
    });
  } catch (error) {
...
async function downloadRequestPDF(req, res) {
  try {
    ...
    // Determine if user can view commercial section
    // Only POD, Discipline Unit, Discipline Manager, DODM, and maintenance roles can see commercial
    const commercialRoles = ['pod_planner', 'discipline_unit', 'discipline_manager', 'dodm', 'maintenance_manager', 'assistant_discipline_manager'];
    const includeCommercial = commercialRoles.includes(req.user.role);
    
    await generateMRFPDF(id, filePath, req.user, includeCommercial);
...
// ===================================
// DOWNLOAD MRF (Final Phase)
// ===================================
async function downloadMRF(req, res) {
  try {
    const { id } = req.params;
    
    console.log(`📄 Generating MRF for request ID: ${id}`);
    ...
    // Allow MRF download after DM approval (COMMERCIAL_REVIEW stage) or later stages
    const allowedStages = ['COMMERCIAL_REVIEW', 'COMMERCIAL_APPROVED', 'MATERIAL_DELIVERY', 'MATERIAL_RECEIVED', 'CLOSED', 'COMPLETED'];
    if (!allowedStages.includes(request.workflow_stage) && request.status !== 'Approved') {
      return res.status(400).json({ 
        error: true, 
        message: `MRF can only be generated after Discipline Manager approval. Current stage: ${request.workflow_stage}, Status: ${request.status}` 
      });
    }
    ...
    // Determine if user can view commercial section
    // Field workers (requestor, tech coord, asst mgr, area manager) should NOT see commercial details
    ...
    const commercialRoles = ['pod_planner', 'discipline_unit', 'discipline_manager', 'dodm', 'maintenance_manager', 'assistant_discipline_manager'];
    const includeCommercial = !fieldWorkerRoles.includes(req.user.role) && commercialRoles.includes(req.user.role);
    
    try {
      await generateMRFPDF(id, filePath, req.user, includeCommercial);
      console.log(`✅ PDF generated successfully (Commercial: ${includeCommercial})`);
    } catch (pdfError) {
      console.error('❌ PDF generation error:', pdfError);
      console.error('Error stack:', pdfError.stack);
      return res.status(500).json({ 
        error: true, 
        message: 'Failed to generate MRF PDF: ' + pdfError.message 
      });
    }
    ...
    // Send PDF
    res.download(filePath, `Invoice_${request.mrf_number}.pdf`, (err) => {
      if (err) {
        console.error('❌ Invoice download error:', err);
        if (!res.headersSent) {
          res.status(500).json({ error: true, message: 'Failed to download MRF: ' + err.message });
        }
      }
      // Delete temp file after download
      setTimeout(() => {
        fs.unlink(filePath, (unlinkErr) => {
          if (unlinkErr) console.error('Failed to delete temp MRF:', unlinkErr);
        });
      }, 1000);
    });
    ...
    query(
      'INSERT INTO activity_logs (user_id, action, entity_type, entity_id, details) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, 'MRF_DOWNLOADED', 'material_request', id, `Downloaded MRF for ${request.mrf_number}`]
    ).catch(err => console.error('Failed to log activity:', err));

  } catch (error) {
    console.error('❌ MRF generation error:', error);
    console.error('Error stack:', error.stack);
    res.status(500).json({ 
      error: true, 
      message: 'Failed to generate MRF: ' + (error.message || 'Unknown error') 
    });
  }
}
...
// ===================================
async function confirmMaterialDelivery(req, res) {
  try {
    const { id } = req.params;
    const { line_items } = req.body; // Array of { line_id, received_quantity, material_delivered }
    const user = req.user;
    ...
    if (request.user_id !== user.id && user.role !== 'admin') {
      return res.status(403).json({ error: true, message: 'Only the requestor can confirm material delivery' });
    }

    // Check if request is at MATERIAL_DELIVERY or MATERIAL_RECEIVED stage
    if (request.workflow_stage !== 'MATERIAL_DELIVERY' && request.workflow_stage !== 'MATERIAL_RECEIVED') {
      return res.status(400).json({ 
        error: true, 
        message: `Materials must be delivered before confirmation. Current stage: ${request.workflow_stage}` 
      });
    }

    if (!line_items || !Array.isArray(line_items) || line_items.length === 0) {
      return res.status(400).json({ error: true, message: 'Line items with received quantities are required' });
    }

    let allMaterialsDelivered = false;

    await transaction(async (client) => {
      // Update received quantities for each line item
      for (const item of line_items) {
        const { line_id, received_quantity, material_delivered } = item;
        
        if (material_delivered) {
          await client.query(
            `UPDATE material_request_lines 
             SET received_quantity = $1, 
                 actual_delivery_date = CURRENT_DATE
             WHERE id = $2 AND request_id = $3`,
            [received_quantity || 0, line_id, id]
          );
        }
      }
      ...
      const linesResult = await client.query(
        `SELECT COUNT(*) as total, 
                SUM(CASE WHEN received_quantity >= quantity THEN 1 ELSE 0 END) as delivered_count
         FROM material_request_lines 
         WHERE request_id = $1`,
        [id]
      );

      const { total, delivered_count } = linesResult.rows[0];
      allMaterialsDelivered = parseInt(delivered_count) === parseInt(total);

      // Update workflow stage
      let nextStage = 'MATERIAL_RECEIVED';
      ...
      await client.query(
        ...
        [
          ...
          user.role,
          allMaterialsDelivered ? 'All materials confirmed as delivered' : 'Partial materials confirmed'
        ]
      );
    });
    ...
    res.json({
      success: true,
      message: allMaterialsDelivered 
        ? 'All materials confirmed as delivered. Request closed.' 
        : 'Material delivery confirmed. Waiting for remaining items.'
    });
  } catch (error) {
    console.error('Confirm material delivery error:', error);
    res.status(500).json({ error: true, message: 'Failed to confirm material delivery' });
  }
}
