...
async function login(req, res) {
  try {
    ...
    // Get user by email (include all fields for password verification)
    const result = await query(
    ...
    // Generate JWT token - JWT_SECRET must be set in environment variables
    ...
    await query(
      ...
      [user.id, 'USER_LOGIN', req.ip || 'unknown']
    );

    // Return user data (without password_hash, but include all workflow fields)
